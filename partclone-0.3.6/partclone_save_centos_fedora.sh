#! /bin/bash
echo "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-"
echo "                        Welcome to partclone save centos and fedora                        "
echo "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-"
echo
...
{
    echo "check_save_partition to save centos server system  and fedora system when partclone save"
    ...
    if [[ ! -z $(cat /tmp/sdd_is_save_partition.info | grep sdd7) ]]
    then
        type=sdd
    elif [[ ! -z $(cat /tmp/sdc_is_save_partition.info | grep sdc7) ]]
    then
        type=sdc
    elif [[ ! -z $(cat /tmp/sdb_is_save_partition.info | grep sdb7) ]]
    then
        type=sdb
    elif [[ ! -z $(cat /tmp/sda_is_save_partition.info | grep sda7) ]]
    then
        type=sda
    fi
}
...
do_partclone_save_sda()
{
	...
	fsck /dev/sda5
	partclone.extfs -c -d -s /dev/sda5 -o $save_image_directory/fedora_boot_partition.img


	# partclone save fedora_root_partition.img
	rm -fr $save_image_directory/fedora_root_partition.img
	fsck /dev/sda6
	partclone.extfs -c -d -s /dev/sda6 -o $save_image_directory/fedora_root_partition.img
...
do_partclone_save_sdb()
{
	...
	rm -fr $save_image_directory/fedora_boot_partition.img
	fsck /dev/sdb5
	partclone.extfs -c -d -s /dev/sdb5 -o $save_image_directory/fedora_boot_partition.img


	# partclone save fedora_root_partition.img
	rm -fr $save_image_directory/fedora_root_partition.img
	fsck /dev/sdb6
	partclone.extfs -c -d -s /dev/sdb6 -o $save_image_directory/fedora_root_partition.img
...
do_partclone_save_sdc()
{
	...
	rm -fr $save_image_directory/fedora_boot_partition.img
	fsck /dev/sdc5
	partclone.extfs -c -d -s /dev/sdc5 -o $save_image_directory/fedora_boot_partition.img


	# partclone save fedora_root_partition.img
	rm -fr $save_image_directory/fedora_root_partition.img
	fsck /dev/sdc6
	partclone.extfs -c -d -s /dev/sdc6 -o $save_image_directory/fedora_root_partition.img
...
do_partclone_save_sdd()
{
	...
	rm -fr $save_image_directory/fedora_boot_partition.img
	fsck /dev/sdd5
	partclone.extfs -c -d -s /dev/sdd5 -o $save_image_directory/fedora_boot_partition.img


	# partclone save fedora_root_partition.img
	rm -fr $save_image_directory/fedora_root_partition.img
	fsck /dev/sdd6
	partclone.extfs -c -d -s /dev/sdd6 -o $save_image_directory/fedora_root_partition.img
